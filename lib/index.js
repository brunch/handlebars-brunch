...
module.exports = HandlebarsCompiler = (function() {
  ...
  function HandlebarsCompiler(config) {
    ...
    this.config = config;
    handlebarsConfig = (_ref = this.config.plugins) != null ? _ref.handlebars : void 0;
    if (handlebarsConfig) {
  ...
  HandlebarsCompiler.prototype.compile = function(data, path, callback) {
    ...
    try {
      ...
      return result = this.namespace ? (ns = this.namespace, key = path.replace(/^.*templates\//, '').replace(/\..+?$/, ''), "if (typeof " + ns + " === 'undefined'){ " + ns + " = {} }; " + ns + "['" + key + "'] = " + source) : umd(source);
    } catch (_error) {
      err = _error;
      return error = err;
    } finally {
